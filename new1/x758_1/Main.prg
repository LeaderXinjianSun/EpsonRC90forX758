...
UnloadOperate_Pass:
	...
	Call RoutePlanThenExe(CurPosition_Num, TargetPosition_Num)
	Accel 50, 50
	If IsCheckINI And isScanCheckFlag Then
		Go ChangeHandL
	Else
		Go FinalPosition
	EndIf
...
'			Print "请将机械手调整为左手姿势"
'		EndIf
		If CU(Here) > -180 And CU(Here) < 360 Then
		
		Else
			HomeSuccessFlage = False
			Print "角度偏差超限，当前角度为 " + Str$(CU(Here))
			MsgSend$ = "角度偏差超限，当前角度为" + Str$(CU(Here))
		EndIf
		If CX(Here) > -50 And CX(Here) < 0 Then
		
		Else
			HomeSuccessFlage = False
			Print "X方向偏差超限，当前X坐标为 " + Str$(CX(Here))
			MsgSend$ = "X方向偏差超限，当前X坐标为 " + Str$(CX(Here))
		EndIf
		If CY(Here) > 150 And CY(Here) < 350 Then
		
		Else
			HomeSuccessFlage = False
			Print "Y方向偏差超限，当前Y坐标为 " + Str$(CY(Here))
			MsgSend$ = "Y方向偏差超限，当前Y坐标为 " + Str$(CY(Here))
		EndIf
...
'						EndIf
				...
				Case "IsReleaseFailContinue"
					...
					Send
				Case "isScanCheckFlag"
					Select CmdRevStr$(1)
						Case "True"
							isScanCheckFlag = True
						Case "False"
